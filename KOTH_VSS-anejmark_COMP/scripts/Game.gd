...
const TEAM_COLORS := {
	Team.BLUE: Character.SkinColor.BLUE,
	Team.YELLOW: Character.SkinColor.YELLOW,
	Team.GREEN: Character.SkinColor.GREEN,
	Team.RED: Character.SkinColor.RED
}

const TEAM_SPAWNS := {
	Team.BLUE: Vector3(1080, 3, 1080),
	Team.YELLOW: Vector3(920, 3, 1080),
	Team.GREEN: Vector3(920, 3, 920),
	Team.RED: Vector3(1080, 3, 920)
}
...
func Move_to_Arena(id: int, player_info: Dictionary):
	...
	# Spawn using skin-based function
	var skin_enum = player_info["skin"]
...
func Spawn_on_Arena(skin: int) -> Vector3:
	...
	# Base spawn positions by skin
	if skin == Character.SkinColor.BLUE:
		base = Vector3(1080, 3, 1080)
	elif skin == Character.SkinColor.YELLOW:
		base = Vector3(920, 3, 1080)
	elif skin == Character.SkinColor.GREEN:
		base = Vector3(920, 3, 920)
	elif skin == Character.SkinColor.RED:
		base = Vector3(1080, 3, 920)
	else:
		base = Vector3(0, 10, 0) # fallback

	# Random offsets ±1 to ±4 (integer steps)
	var random_x = base.x + randi_range(-4, 4)
	var random_z = base.z + randi_range(-4, 4)
	
	return Vector3(random_x, base.y, random_z)
...
func startGame():
	...
	for id in player_ids:
		var team := team_index % 4
		player_teams[id] = team
		team_index += 1

		var skin = TEAM_COLORS[team] # get skin from team
		var spawn_pos = Spawn_on_Arena(skin) # random offset included

		rpc("rpc_spawn_player_in_arena", id, skin, spawn_pos)


@rpc("authority", "call_local")
func rpc_spawn_player_in_arena(peer_id: int, skin: int, spawn_pos: Vector3):
	if not players_container.has_node(str(peer_id)):
		return

	var player := players_container.get_node(str(peer_id)) as Character
	if not player:
		return
