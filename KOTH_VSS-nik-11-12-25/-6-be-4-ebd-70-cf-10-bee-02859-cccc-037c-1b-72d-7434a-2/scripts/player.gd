...
func _input(event):
	if event.is_action_pressed("damage"): # Enter/Space by default
		update_health(-10)
		
	if event.is_action_pressed("damage"):
		update_health(-10)

	if event.is_action_pressed("spawn_potion"): # map this to "L"
		_spawn_potion()

@rpc("any_peer", "reliable")
func _spawn_potion():
	print("DEBUG potion_scene =", potion_scene)
	if not is_multiplayer_authority():
		return
	if not potion_scene:
		push_warning("potion_scene is not assigned!")
		return

	var potion = potion_scene.instantiate()  # works now
	potion.global_transform.origin = global_transform.origin + -global_transform.basis.z * 2
	get_tree().current_scene.add_child(potion)

	potion.used.connect(_on_potion_used)

func _on_potion_used(player_id):
