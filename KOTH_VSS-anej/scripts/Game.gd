...
func startGame():
	...
	for id in player_ids:
		...
		var spawn_pos = TEAM_SPAWNS[team]

		rpc("rpc_spawn_player_in_arena", id, team_color, spawn_pos)
...
func get_random_spawn(team: Team) -> Vector3:
	var base = TEAM_SPAWNS[team]
	# Generate random 2D offset
	var offset_2d = Vector2.from_angle(randf() * TAU) * 10
	# Apply offset in XZ plane
	return base + Vector3(offset_2d.x, 0, offset_2d.y)
