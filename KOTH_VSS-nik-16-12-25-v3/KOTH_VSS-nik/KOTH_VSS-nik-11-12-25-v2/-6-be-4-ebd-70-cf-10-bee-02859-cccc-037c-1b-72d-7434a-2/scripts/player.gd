...
var active_potion: Node3D = null
var is_charging_potion := false
var potion_charge_start := 0.0
var last_potion_use_time := -999.0
...
@export var potion_windup_time := 3.0
@export var potion_cooldown_time := 20.0
...
func _ready():
	...
	active_potion.visible = false
	active_potion.used.connect(_on_potion_used)
...
func _process(_delta):
	...
	if is_charging_potion:
		var elapsed := (Time.get_ticks_msec() - potion_charge_start) / 1000.0
		if elapsed >= potion_windup_time:
			is_charging_potion = false
			use_potion()
...
@rpc("any_peer", "reliable")
func request_heal(amount):
	if not is_multiplayer_authority():
		return
	current_health = clamp(current_health + amount, 0, max_health)
	health_bar.value = current_health
func request_potion_heal():
	if not multiplayer.is_server():
		return

	var now := Time.get_ticks_msec() / 1000.0
	if now - last_potion_use_time < potion_cooldown_time:
		return

	last_potion_use_time = now
	apply_potion_heal()

func apply_potion_heal():
	current_health = clamp(current_health + 20, 0, max_health)
	health_bar.value = current_health
...
func _input(event):
	...
	if event.is_action_pressed("spawn_potion"): # L key
		if not can_use_potion():
			return 

		if active_potion:
			active_potion.visible = true
			active_potion.global_transform.origin = global_transform.origin + -global_transform.basis.z * 1.5
			
	if event.is_action_pressed("use"):
		start_potion_charge()
	
	if event.is_action_released("use"):
		cancel_potion_charge()
	
func start_potion_charge():
	if not active_potion or not active_potion.visible:
		return
	...
	is_charging_potion = true
	potion_charge_start = Time.get_ticks_msec()

func cancel_potion_charge():
	if not is_charging_potion:
		return
...
func _on_potion_used():
	...
	last_potion_use_time = Time.get_ticks_msec() / 1000.0

	if multiplayer.is_server():
		apply_potion_heal()
	else:
		request_potion_heal.rpc_id(1)
...
func can_use_potion() -> bool:
	var now := Time.get_ticks_msec() / 1000.0
	return now - last_potion_use_time >= potion_cooldown_time
	


@rpc("any_peer", "reliable")
func rpc_take_damage(amount := 10):
	if not multiplayer.is_server():
		return
...
func apply_damage(amount: int):
	...
	# send updated health to the owning client
	rpc_sync_health.rpc_id(get_multiplayer_authority(), current_health)

@rpc("authority", "reliable")
func rpc_sync_health(new_health: int):
